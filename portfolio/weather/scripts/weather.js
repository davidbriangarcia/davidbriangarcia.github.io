...
async function getWeatherData(lat, lng) {
    ...
    if (cachedData) {
        ...
        const isCacheValid = (new Date().getTime() - timestamp) < 3600000; // 1 hour
        if (isCacheValid) {
            return data;
        }
    }
    ...
    const combinedData = { weatherData, historicalData };
    localStorage.setItem(cacheKey, JSON.stringify({ data: combinedData, timestamp: new Date().getTime() }));
...
async function updateWeatherData(lat, lng) {
    try {
        ...
        const { weatherData, historicalData } = await getWeatherData(lat, lng);
